...
void DEBUG_UART_Handler(char ch) {}
#endif
...
int main(void) {
	...
	sConfig.Channel = ADC_CHANNEL_1; //todo: IN1, seed gen, pozbyc sie na rzecz in4 od razu ew..
	sConfig.Rank = 1;
	...
	sConfig.Channel = ADC_CHANNEL_4; //IN4, BAT_IN
	if (HAL_ADC_ConfigChannel(&hadc1, &sConfig) != HAL_OK) {
		Error_Handler();
	}
...
static void MX_GPIO_Init(void) {
	...
	/*Configure GPIO pins : PA0 PA6 PA7 PA8
	 ...
	 PA15 */
	GPIO_InitStruct.Pin =
			GPIO_PIN_0 | GPIO_PIN_6 | GPIO_PIN_7 | GPIO_PIN_8 | GPIO_PIN_9
